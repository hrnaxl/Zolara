...
function sbPatch(id, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({ hostname: SUPABASE_HOST, path: "/rest/v1/gift_cards?id=eq." + id, method: "PATCH", headers }, res => {
...
module.exports = async function handler(req, res) {
  ...
  try {
    const r = await sbPatch(cardId, { status: "active", payment_status: "paid" });
