// Zolara activate-gift-card API — marks a digital card active after payment
...
function sbPatch(id, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(
      { hostname: SB_URL, path: "/rest/v1/gift_cards?id=eq." + id, method: "PATCH", headers },
      (res) => {
        let buf = "";
        res.on("data", (c) => (buf += c));
        res.on("end", () => {
...
module.exports = async function handler(req, res) {
  ...
  try {
    const r = await sbPatch(giftCardId, { status: "active", payment_status: "paid", payment_ref: paymentRef || null });
    ...
    console.error("activate crash:", err.message);
