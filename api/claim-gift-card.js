...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const updateRes = await fetch(`${SUPABASE_URL}/rest/v1/gift_cards?id=eq.${card.id}`, {
      method: "PATCH",
      headers: { ...headers, "Prefer": "return=representation" },
      body: JSON.stringify({ payment_status: "pending_pickup", buyer_name: buyerName || null, buyer_email: buyerEmail || null, buyer_phone: buyerPhone || null, recipient_name: buyerName, notes: `RESERVED FOR PICKUP. Buyer: ${buyerName} | Phone: ${buyerPhone} | Ref: ${paymentRef} | Date: ${new Date().toISOString()}` }),
    });
    const updateData = await updateRes.json();
    return res.status(200).json({ claimed: true, card: { ...card, ...(Array.isArray(updateData) ? updateData[0] : updateData) }, message: "Card reserved successfully" });
  } catch (err) {
