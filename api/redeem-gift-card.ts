...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Only add redemption_count if column exists
    ...
    console.log("Redeem update:", patch.status, JSON.stringify(patch.data).slice(0, 200));
    ...
    return res.status(200).json({
      ...
      redemptionCount,
      fullyUsed,
      card: Array.isArray(patch.data) ? patch.data[0] : patch.data,
    });
  } catch (err: any) {
